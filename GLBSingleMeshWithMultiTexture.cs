...
{
    // 1 xdo - 1 메쉬 + n 텍스쳐를 입혀 glb 파일 하나로 만드는 클래스
    // GLTF, GLB에 있던 attribute(ObjBox, key, color, altitude 등의 값은 가져오지 않았다. 추가할거면 기존의 것을 가져와서 추가시키기만 하면 끝)
    // 텍스쳐 건드리는 쪽에서 막힘 -> texture atlas 이용하여 작업진행해야 할 듯
    public class GLBSingleMeshWithMultiTexture
    {
        ...
        List<List<List<float>>> bin_texture = new List<List<List<float>>>();
        ...
        int byteLength = 0;
        int meshIndex = 0;
        public GLBSingleMeshWithMultiTexture(GLBInfo[] gltf_array, string key)
        {
            key = key + "_single_multiTexture";
            mergeBin(gltf_array);
        ...
        private void mergeBin(GLBInfo[] gltf_array)
        {
            ...
            for (int i = 0; i < gltf_array.Length; i++)
            {
                ...
                List<List<float>> binTextureContainer = new List<List<float>>();
                ...
                for (int j = 0; j < gltf_array[i].glb.gltf.xdo.faceNum; j++)
                {
                    ...
                    binTextureContainer.Add(bin_textures);
                ...
                bin_texture.Add(binTextureContainer);
            ...
            print3DMatrix(bin_texture);
        }
        private void print3DMatrix(List<List<List<float>>> texture)
        {
            for(int i = 0; i < texture.Count; i++)
            {
                ...
                {
                    Console.Write(texture[i][j].Count + " ");
                }
                Console.WriteLine("");
            }
        }
        ...
        private int makeMeshes(JObject superjson, GLBInfo[] gltf_array, bool debug, int meshcount)
        {
            ...
            for (int i = 0; i < meshcount; i++)
            {
                ...
                {
                    POSITION = 1 + i + meshIndex,
                    NORMAL = 2 + i + meshIndex,
                    _BATCHID = 3 + i + meshIndex
                });
                texCoordIndex += 3;
                for(int j = 0; j < bin_texture[i].Count; j++)
                {
                    attribute_token["TEXCOORD_" + j] = 4 + i + j + meshIndex;
                    texCoordIndex++;
                }
                JToken primitives_token = JObject.FromObject(new
                {
                    attributes = attribute_token,
                    indices = 0 + i + meshIndex,
                    material = i
                });
                // texCoordIndex++;

                meshIndex += texCoordIndex; 
        ...
        private void makeGLB(JObject superjson, GLBInfo[] gltf, string key, int total_offset, int meshcount)
        {
            ...
            StreamWriter s1w = new StreamWriter(gltf[0].filePath + key + "_singleMeshWithMultiTexture.gltf");
            s1w.Write(superjson.ToString());
            s1w.Close();
